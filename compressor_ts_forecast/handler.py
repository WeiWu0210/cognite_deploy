...
def handle(client, data=None, secrets=None, function_call_info=None):
    ...
    create_and_save_time_series_data(
        client, fcst_df[["yhat"]], f"{ts_exids[0]}_Forecast_Trend", data_set_id=data_set_id
    )
    create_and_save_time_series_data(
        client, fcst_df[["yhat_lower"]], f"{ts_exids[0]}_Forecast_Lower", data_set_id=data_set_id
    )
    create_and_save_time_series_data(
        client, fcst_df[["yhat_upper"]], f"{ts_exids[0]}_Forecast_Upper", data_set_id=data_set_id
    )
